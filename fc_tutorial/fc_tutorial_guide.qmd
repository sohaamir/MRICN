...
![Atlas](/Users/aamirsohail/Documents/GitHub/MRICN/assets/atlas_view.png){width=600}

To save the seed, click the save symbol which is the first of three icons on the
bottom left of the window. 

![Save PCC](../assets/save_atlas.png){width=550}
...
In FSLeyes click File ➜ Add from file to compare PCC.nii.gz (before binarization) and PCC_bin.nii.gz (after binarization). You should note that the signal values are 
all 1.0 for the binarized PCC.

![Binary mask](../assets/binarized.png){width=300}
...
![FEAT Data](../assets/data.png){width=350}
...
![Pre-stats](../assets/pre_stats.png){width=350}
...
![GLM](../assets/glm_window.png){width=300, height=500}
...
**Post-stats**: Nothing needs to be changed here.
...
![BEAR Files](../assets/bear_files.png){width=500, height=90}
...
![sub1_brain](../assets/sub1_brain.png){width=300, height=1000}
...
![sub2_feat](../assets/sub2_feat.png){width=300, height=1000}

![sub2_feat](../assets/sub3_feat.png){width=300, height=1000}

### Running the Group-level analysis

Ok, so now we have our FEAT directories for all three subjects, we can run the 
group level analysis. Open Feat by running `Feat &` in your `SBC` directory.
...
![group_data](../assets/group_data.png){width=350, height=300}
...
![group_data](../assets/selection.png){width=350, height=300}

**Stats**

Click Full model setup. In the General Linear Model window, name the model 'PCC' and make sure the EVs are all 1s. The interface should look like this:

![group_data](../assets/glm.png){width=300, height=300}
...
This should only take about 2-3 minutes to run. You can load the `report.html` 
through the file browser as you did for the individual subjects. Click on the 
'Results' tab, and then on 'Lower-level contrast 1 (PCC)'. Your results should 
look like this: 

![group_data](../assets/group_results.png){width=250, height=300}
...
When in fsleyes, open up the standard brain (Navigate to the top menu and click on 'File ➜ Add standard' and select `MNI152_T1_2mm_brain.nii.gz`. Then add in our 
...
![group_fsleyes](../assets/group_fsleyes.png){width=500, height=300}
...
![group_fsleyes](../assets/axial_slice.png){width=, height=300}

![group_fsleyes](../assets/axial_feat.png){width=500, height=300}
...
Right superior lateral occipital cortex (sLOC)

![group_fsleyes](../assets/sloc.png){width=300, height=300}

Posterior cingulate cortex (PCC) / precuneus

![group_fsleyes](../assets/pcc_precuneus.png){width=300, height=300}

We can also use Neurosynth, a website where you can get the resting-state 
functional connectivity of any voxel location of brain region. It does this by 
extracting data from studies and performing a meta-analysis associated with 
your voxel/region of interest. 

If you click the following link: 

https://neurosynth.org/analyses/terms/posterior%20cingulate/

You will see regions demonstrating significant functional connectivity with the 
posterior cingulate. If you type in (46, -70, 32) into Neurosynth, and then into
the MNI co-ordinates in fsleyes (not as voxels, because Neurosynth works with 
MNI space), you can see that in both cases the right superior lateral occipital
cortex is activated. Note that the orientations of left and right are different.

This is a great result given that we only have three subjects!

![neurosynth](../assets/neurosynth.png){width=300, height=450}
![group_fsleyes](../assets/fsleyes_sloc.png){width=400, height=450}
