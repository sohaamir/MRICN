...
This tutorial is based upon [FSL fMRI Resting State Seed-based Connectivity](https://neuroimaging-core-docs.readthedocs.io/en/latest/pages/fsl_fmri_restingstate-sbc.html) by Dianne Paterson at the University of Arizona, which has been 
adapted to run on the BEAR systems at the University of Birmingham, with additional material added.
...
default mode network (DMN) that is commonly observed in resting-state data. To do this, we
will extract a mean-timeseries for a PCC seed region for each participant, running
single-subject level analyses, one manually and bash scripting the other two. We will then
run a group-level analysis using the single-level results. Finally, we will also take a
look at [Neurosynth](https://neurosynth.org/) and how it can be used to generate
...
Now when you run `pwd` you should see:
...
2) When the image is open, click on `Settings ➜ Ortho View 1 ➜ Atlas panel`
3) Click `Settings ➜ Ortho View 1 ➜ Atlas panel`. An atlas panel opens on the bottom right.
4) Select `Atlas search` (red box, lower middle).
5) Ensure Harvard-Oxford Cortical Structural Atlas is selected (purple highlighting).
6) Type `cing` in the search box and check the Cingulate Gyrus, posterior division (lower right) so that it is overlaid on the standard brain. (The full name may 
be obscured, but you can always check which region you have loaded by looking at 
...
In FSLeyes click File ➜ Add from file to compare PCC.nii.gz (before binarization) and
PCC_bin.nii.gz (after binarization). You should note that the signal values are 
all 1.0 for the binarized PCC.
...
1) Keep the Number of original EVs is 1.
...
```
pwd # make sure you are in your MRICN directory
cd sub1.feat
cat design.fsf
```

That it acts as a large function with its many variables corresponding to the 
options that we choose when setting up in the GUI. We just need to change three 
of these (the three mentioned above):
...
You can run the code (from your SBC directory) by typing:

`bash run_feat.sh`

It should take about 5-10 minutes to run. If you have a look at the `report.html`
file for both, they should look like this:
...
group level analysis. Open Feat by running `Feat &` in your `SBC` directory. Here 
is the group-level FEAT setup:
...
<br>
<br>
5) In 'Selection' stay in your current directory (it should be SBC), and in the 
bottom bar, type in `PCC_group` at the end of the file path. This is what mine
looks like: 
...
Click Full model setup. In the General Linear Model window, name the model 'PCC' and make
sure the EVs are all 1s. The interface should look like this:
...
outputted data. Close your terminal, open a new one, go to your `SBC` directory 
...
When in fsleyes, open up the standard brain (Navigate to the top menu and click on 'File ➜
...
what regions of the brain are they? Let's go through two ways by which we can 
work this out. 

We can simply just overlap an atlas on the image and see which regions the 
activated voxels fall under. To do this, navigate to the top menu and click on 'Settings ➜ Ortho View 1 ➜ Atlases'. Then at the bottom middle of the window, 
select the 'Harvard-Oxford Cortical Structural Atlas' and on the window directly
next to it on the right, click 'Show/Hide'. The atlas should have loaded up but 
is blocking the voxels. Change the Opacity to about half-way. 

We can now see that significant voxels of activity are mainly found in the: 
...
We can also use [Neurosynth](https://neurosynth.org/), a website where you can get the
resting-statefunctional connectivity of any voxel location or brain region. It does this
by extracting data from studies and performing a meta-analysis on brain imaging studies
that have results associated with your voxel/region of interest. 

If you click the following link: 

[https://neurosynth.org/analyses/terms/posterior%20cingulate/](https://neurosynth.org/analyses/terms/posterior%20cingulate/)


You will see regions demonstrating significant connectivity with the posterior cingulate.
If you type `(46, -70, 32)` as co-ordinates, and then into the MNI co-ordinates section in
fsleyes, **not as voxels, because Neurosynth works with MNI space**, you can see that in
both cases the right superior lateral occipital cortex is activated. Note that the
orientations of left and right are different.
